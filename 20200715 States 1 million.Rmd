...
row <- row %>% group_by(State) %>% mutate(`Daily Confirmed Cases` = (lag(`Daily cases`, 3) + lag(`Daily cases`, 2) + lag(`Daily cases`, 1) + `Daily cases` + lead(`Daily cases`, 1) + lead(`Daily cases`, 2) + lead(`Daily cases`, 3))/7)
...
```{r}
classification <- row %>% filter(Date == "2020-07-11" | Date == "2020-06-27") %>% arrange(State, Date) %>% group_by(State) %>% mutate(diff = `Daily Confirmed Cases` - lag(`Daily Confirmed Cases`), pct = 100*diff / `Daily Confirmed Cases`)
```
...
```{r}
full %>% filter(Date >= "2020-04-01" & Status == "Rising") %>% ggplot() + geom_area(aes(x = Date, y = `Daily Confirmed Cases`), alpha = 0.4, fill = "#9c261e", color = "#3b0805", size = 0.2) + facet_wrap(~`State name`, scales = "free") + theme_minimal() + theme(axis.text.x  =  element_blank(), axis.text.y  = element_text(size = 5),  panel.grid =  element_blank(), axis.title.x = element_blank(), strip.text = element_text(size = 6))
ggsave("rising_scale.png")

full %>% filter(Date >= "2020-04-01" & Status != "Rising") %>% ggplot() + geom_area(aes(x = Date, y = `Daily Confirmed Cases`), alpha = 0.5, fill = "#63b877", color = "#053b12", size = 0.2) + facet_wrap(~`State name`, scales = "free", ) + theme_minimal() + theme(axis.text.x  =  element_blank(), axis.text.y  = element_text(size = 5),  panel.grid =  element_blank(), axis.title.x = element_blank(), strip.text = element_text(size = 8))
ggsave("stable_scale.png")
```
